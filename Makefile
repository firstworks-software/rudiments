...
distclean: clean
	$(RM) configure~ \
		...
		libtool \
		rudiments.pc \
		include/Makefile include/rudiments/private/config.h \
		include/rudiments/private/config.h.in~ \
		bin/rudiments-config \
		src/Makefile \
		msvc/setupx86/rudiments.opensdf \
		msvc/setupx86/rudiments.sdf \
		msvc/setupx86/rudiments.suo \
		msvc/setupx86/setupx86.vdproj \
		msvc/setupx64/rudiments.opensdf \
		msvc/setupx64/rudiments.sdf \
		msvc/setupx64/rudiments.suo \
		msvc/setupx64/setupx64.vdproj \
		configure.lineno \
		configure~ \
		test/certificates/config/ca.cnf \
