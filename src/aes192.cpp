...
aes192::aes192() : encryption() {
	pvt=new aes192private;
	pvt->_context=NULL;
}
...
size_t aes192::getKeySize() {
	return 24;
}

size_t aes192::getIvSize() {
	return AES_BLOCK_SIZE;
}

const byte_t *aes192::getEncryptedData() {
	return getData(true);
}

const byte_t *aes192::getDecryptedData() {
	return getData(false);
}

const byte_t *aes192::getData(bool encrypt) {
	...
	// decide on a block cipher mode
		...
		switch (getBlockCipherMode()) {
			#ifdef RUDIMENTS_HAS_EVP_AES_192_CBC
			case BLOCK_CIPHER_MODE_CBC:
				mode=EVP_aes_192_cbc();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_CFB1
			case BLOCK_CIPHER_MODE_CFB1:
				mode=EVP_aes_192_cfb1();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_CFB8
			case BLOCK_CIPHER_MODE_CFB8:
				mode=EVP_aes_192_cfb8();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_CFB128
			case BLOCK_CIPHER_MODE_CFB128:
				mode=EVP_aes_192_cfb128();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_ECB
			case BLOCK_CIPHER_MODE_ECB:
				mode=EVP_aes_192_ecb();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_OFB
			case BLOCK_CIPHER_MODE_OFB:
				mode=EVP_aes_192_ofb();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_GCM
			case BLOCK_CIPHER_MODE_GCM:
				mode=EVP_aes_192_gcm();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_CCM
			case BLOCK_CIPHER_MODE_CCM:
				mode=EVP_aes_192_ccm();
				break;
			#endif
			#ifdef RUDIMENTS_HAS_EVP_AES_192_CTR
			case BLOCK_CIPHER_MODE_CTR:
				mode=EVP_aes_192_ctr();
				break;
			#endif
			default:
				encryption::setError(
...
void aes192::setError(int32_t err) {
	#if defined(RUDIMENTS_HAS_SSL)
