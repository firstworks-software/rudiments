...
/* Rudiments: the hostap "u8"/"u32" types, the os_*() wrappers, the
 * WPA_GET_BE32()/WPA_PUT_BE32() accessors and the des3_key_s struct come from
 * hostap's includes.h/common.h/des_i.h.  They are replaced here so that this
 * file stands alone. */
...
static void deskey(const unsigned char *key, int decrypt, uint32_t *keyout)
{
...
static inline void des_key_setup(const unsigned char *key, uint32_t *ek,
					uint32_t *dk)
{
	deskey(key, 0, ek);
	deskey(key, 1, dk);
...
static inline void des_block_encrypt(const unsigned char *plain,
					const uint32_t *ek,
					unsigned char *crypt)
{
	uint32_t work[2];
	work[0] = GETU32BE(plain);
	work[1] = GETU32BE(plain + 4);
	desfunc(work, ek);
	PUTU32BE(crypt, work[0]);
	PUTU32BE(crypt + 4, work[1]);
}


static inline void des_block_decrypt(const unsigned char *crypt,
					const uint32_t *dk,
					unsigned char *plain)
{
	uint32_t work[2];
	work[0] = GETU32BE(crypt);
	work[1] = GETU32BE(crypt + 4);
	desfunc(work, dk);
	PUTU32BE(plain, work[0]);
	PUTU32BE(plain + 4, work[1]);
}


static inline void des3_key_setup(const unsigned char *key,
...
static inline void des3_encrypt(const unsigned char *plain,
...
static inline void des3_decrypt(const unsigned char *crypt,
					const struct des3_key_s *key,
					unsigned char *plain)
{
	uint32_t work[2];
