...
bool hostentry::open(const char *hostname) {
	return open(hostname,NULL,0,0);
}

bool hostentry::open(const char *address, int32_t len, int32_t type) {
	return open(NULL,address,len,type);
}

bool hostentry::open(const char *hostname, const char *address,
