...
class RUDIMENTS_DLLSPEC semaphoreset : public object {
	public:
		...
		/** By default, the destructor will remove a semaphore set
		 *  if it was created during a call to create() or
		 *  createOrAttach().
		 *
		 *  This method allows you to manually override that behavior.
		 *
		 *  If "remove" is true then the destructor will remove the
		 *  semaphore set whehter it was created or not.  If "remove"
		 *  is set false then the destructor will not remove the
		 *  semaphore set, even if it was created.
		 *
		 *  Manually setting "remove" false is useful, for example, if
		 *  an application creates a semaphore set then forks and wants
		 *  to delete the semaphore set in the forked process but does
		 *  not want the semaphore removed from the system.
		 *
		...
		/** Returns true if the semaphore will be removed by the
		 *  destructor and false otherwise. */
		bool	getRemove();
