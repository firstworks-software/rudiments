...
ssize_t filedescriptor::read(ucs2_t *buffer) {
	return highLevelRead((unsigned char *)buffer,sizeof(ucs2_t));
}
...
ssize_t filedescriptor::read(ucs2_t *buffer, size_t length) {
	return highLevelRead((unsigned char *)buffer,length*sizeof(ucs2_t));
}
...
ssize_t filedescriptor::read(ucs2_t *buffer, int32_t sec, int32_t usec) {
	return highLevelRead((unsigned char *)buffer,sizeof(ucs2_t),sec,usec);
}
